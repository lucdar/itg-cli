...
def add_song(args):
    ...
    with TemporaryDirectory() as working_dir:
        ...
        if root == working_dir:
            ...
            # TODO: Find a better default to use. sm file name instead? Or sm.title?
            ...
            if zip_name == "":
                ...
                for file in os.listdir(root):
                    if file.endswith(".ssc") or file.endswith(".sm"):
                        ...
                        break
            new_root = os.mkdir(os.path.join(working_dir, zip_name))
            for item in filter(lambda x: x!=new_root, os.listdir(root)):
                shutil.move(item, new_root)
            root = new_root
